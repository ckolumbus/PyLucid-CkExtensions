...
from ProtectedGallery.preference_forms import ProtectedGalleryPrefForm
...
class ProtectedGalleryError(Exception):
    """
...
class ProtectedGallery(object):
        ...
        if False: #not is_dir or not is_in_root:
            ...
            if settings.DEBUG or has_change_perm:
                ...
                if has_change_perm: # raise 404 in debug
                    raise ProtectedGalleryError(msg)
            raise Http404(msg)
    ...
    def check_rest_url(self, rest_url):
        ...
        pref_form = ProtectedGalleryPrefForm()
        ...
        for sign in unauthorized_signs:
            if sign and sign in rest_url:
                ...
                LogEntry.objects.log_action(
                    app_label="external_plugins.ProtectedGallery", action=msg1, message=msg
                )
                msg404 = "bad path"
...
def gallery(request, rest_url=""):
    ...
    try:
        config = ProtectedGalleryModel.objects.get(pagetree=pagetree)
    except ProtectedGalleryModel.DoesNotExist, err:
        if request.user.has_perm("gallery.change_gallerymodel"):
            messages.info(request,
                _("ProtectedGallery entry for page: %s doesn't exist, please create it.") % pagetree.get_absolute_url()
            )
            return HttpResponseRedirect(reverse("admin:ProtectedGallery_protectedgallerymodel_add"))
        else:
            messages.warning(request, _("ProtectedGallery is deactivated, yet. Come back later."))
    ...
    try:
        gallery = ProtectedGallery(request, config, rest_url)
    except ProtectedGalleryError, err:
        messages.error(request, "ProtectedGallery error: %s" % err)
